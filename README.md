...
| `fail-on`             | no       | `never`                         | Whether the review fails the check, so a Required check can block the PR. Default `never` (advisory). See [Blocking a PR on the review](#blocking-a-pr-on-the-review) for the gating options, and [Fail-loudly guarantee](#fail-loudly-guarantee) for the one case that always fails regardless of this setting. |
...
- **`LIEN_REVIEW_DOC_PASS=0`** (or `false`): disables the dedicated
  ...
  code. On by default.
- **`LIEN_DOC_TRUTH_V2=off`** (or `0`/`false`): reverts the doc-truth pass to
  its earlier open-findings-list behavior. On (v2) by default; see
...
even under `fail-on: never`. See [Fail-loudly guarantee](#fail-loudly-guarantee)
below.
